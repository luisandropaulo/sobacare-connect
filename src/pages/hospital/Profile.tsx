...
import { hospitalKpis } from "@/data/hospitalMockData";
...
const HospitalProfile = () => {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold">Perfil do Hospital</h1>
      ...
      <div className="flex items-center gap-6">
        ...
        <div>
          <h2 className="text-xl font-bold">{user?.name || "Hospital"}</h2>
          <p className="text-sm text-muted-foreground">{user?.email}</p>
          <span className="inline-block mt-1 text-xs bg-primary/10 text-primary rounded-full px-2 py-0.5">Administração Hospitalar</span>
        </div>
      </div>
      ...
      <Tabs defaultValue="info">
        ...
        <TabsContent value="info" className="space-y-4 mt-4">
          <Card>
            <CardHeader className="flex-row items-center justify-between">
              <CardTitle className="text-base">Dados do Hospital</CardTitle>
              <Button variant="outline" size="sm" onClick={() => editing ? handleSave() : setEditing(true)}>
                {editing ? "Guardar" : "Editar"}
              </Button>
            </CardHeader>
            <CardContent className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2"><Label>Nome</Label><Input defaultValue="Hospital Américo Boavida" disabled={!editing} /></div>
              <div className="space-y-2"><Label>Email</Label><Input defaultValue={user?.email} disabled={!editing} /></div>
              <div className="space-y-2"><Label>Telefone</Label><Input defaultValue="+244 222 300 100" disabled={!editing} /></div>
              <div className="space-y-2"><Label>Localização</Label><Input defaultValue="Luanda, Angola" disabled={!editing} /></div>
              <div className="space-y-2"><Label>Plano</Label><Input defaultValue="Professional" disabled /></div>
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="stats" className="mt-4">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <StatsCard title="Consultas Hoje" value={hospitalKpis.appointmentsToday} icon={CalendarDays} />
            <StatsCard title="Unidades" value={3} icon={Building2} />
          </div>
        </TabsContent>
